...
[`.github/workflows`](.github/workflows/) | [GitHub Action workflow](https://github.com/hegyhati/Diploma_Work_Template/actions) configuration files.

# Compiling the thesis

## Regular way

If you have a linux bases system, simply run `make`, and it will:
 - run `pdflatex`
 - run `bibtex`
 - run `pdflatex` twice
...
In order to have the necessary commands executed my `make` several packages need to be installed, which can alter for each distribution. The [Dockerfile](Docker/Dockerfile) provides these packages for Ubuntu 18.04.

## The Docker way

If you don't want to install these packages on your host system, but have `docker`, simply mount this directoy into a container and do it there:
```
...
If your thesis requires additional LaTeX packages, make sure to either make a new docker image, or extend the [workflow](.github/workflows/build.yml) with installing those packages.

The former way is preferred. Let's say you need the `texlive-games` packages. Here is what you have to do:
...
3. Change the used docker image to yours in the [Makefile](makefile) and the [CI configuration](.github/workflows/build.yml)
...
`\ChapterShort` | `{shorttitle}{title}{name}` | Same as above, but uses `shorttitle` in the TOC and header. (Used, if chapter title is too long.)
...
Command | Parameters | Result
---|---|---
`\Figure` | `{filename}{caption}{options}` | Includes the file `Figures/filename` with `option` options passed to `\includegraphics`, e.g., `width=8cm`. `filename` must contain the extension, and may inlcude a path. It is also used to define a label with id `fig_filename`. `caption` is used for caption. Everything is wrapped in a figure floating block with `htpb` positioning.
`\FigureShort` | `{filename}{shortcaption}{caption}{options}` | Sama as above, only `shortcaption` will appear in the list of figures.
`\Table` | `{labelsuffix}{caption}{columndef}{tablecontent}` | Creates a tabular block with column definition `columndef` (e.g., `r||c|c|c|`), `htpb` positioning,  and with the content in `tablecontent`. General rules of LaTeX tables apply. `caption` will be used as a caption, and a label is defined with an id `tab_labelsuffix`.
